...
export class SVAttributeComponent implements OnInit{
  ...
  private buildMassUpdateSteps(): void {
    this.menuItems = buildMassUpdateMenuItems(this._labels, this._labels.text('S12.MU.STP0', 'Select your Item-SV attribute file change.'));
  ...
  checkGlobal(): boolean {
    ...
    if(this._importService.wb.sheets[0].worksheet.columns.length < 5) {
        this.globalError.push('Item/SV attribute file must contains the following headers: ITEM_CODE, SV_CODE, ATTRIBUTE_CLASS, ATTRIBUTE_CODE, ATTRIBUTE_VALUE'); 
        return false;
    }
