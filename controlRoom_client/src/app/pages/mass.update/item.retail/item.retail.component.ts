 ...
 */
@Component({
    selector: 'item-retail',
    templateUrl: './item.retail.component.html',
    providers: [WarehouseService, WidgetService, MessageService, ExportService, ImportService],
    styleUrls: ['./item.retail.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class ItemRetailComponent implements OnInit{
  ...
  ngOnInit() {
      this.menuItems = [{
              ...
              title: 'Pick your item retail file',
              command: (event: any) => {
                  this.activeIndex = 0;
                  this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Pick your data file item retail', detail: event.item.label});
              }
          },
          {
              id: 'step1',
  ...
  validationChanges() {
    ...
    if (this.checkGlobal()) {
        ...
        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                            '<br><br>'+
                            '<b>item retail change is usually taking between 1 and 3 minutes</b>';
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                       ...
                       this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                           'Step 2/4: Executing item retail mapping change... <br>'+ 
                                            '<br><br>'+
                                            '<b>item retail change is usually taking between 1 and 3 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing item retail mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => { this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Step 4/4: Executing plan', detail:  '"' + this.uploadedFiles[0].name + '" processing plan results collected.'});
                                                    this.msgFinalDisplayed = 'item retail  ' + this.uploadedFiles[0].name + ' - ' + 
  ...
  confirmFile() {
    ...
    this.globalError = [];
    if (this.checkGlobal()) {
        this._importService.wb.sheets[0].worksheet.rows = this._importService.wb.sheets[0].worksheet.rows.map(item => ({
                    ...item,
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>Price list are all recognized</li>' +
                                                            ' <li>Item/sv codes are all recognized</li>' +
                                                            ' <li>Consistency on dates validated</li></ul>'); 
  ...
  checkGlobal(): boolean {
    ...
    if(this._importService.wb.sheets[0].worksheet.columns.length < 5) {
        this.globalError.push('item retail file must contains the following headers: PRICE_LIST, PPG, ITEM_CODE, SV_CODE, RETAIL, MULTIPLE, START_ON, END_ON'); 
        return false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[0].field.toUpperCase() !== 'PRICE_LIST') {
      this.globalError.push('The column A header must be named PRICE_LIST'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'PPG') {
      this.globalError.push('The column B header must be named PPG'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'ITEM_CODE') {
        this.globalError.push('The column C header must be named ITEM_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[3].field.toUpperCase() !== 'SV_CODE') {
        this.globalError.push('The column D header must be named SV_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[4].field.toUpperCase() !== 'RETAIL') {
        this.globalError.push('The column E header must be named RETAIL'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[5].field.toUpperCase() !== 'MULTIPLE') {
        this.globalError.push('The column F header must be named MULTIPLE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[6].field.toUpperCase() !== 'START_DATE') {
        this.globalError.push('The column G header must be named START_DATE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[7].field.toUpperCase() !== 'END_DATE') {
        this.globalError.push('The column H header must be named END_DATE'); 
      result = false;
    }
