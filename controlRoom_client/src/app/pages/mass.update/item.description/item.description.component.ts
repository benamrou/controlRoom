...
@Component({
    ...
    styleUrls: ['./item.description.component.scss', '../../../app.component.scss'],
...
export class ItemDescriptionComponent implements OnInit{
  ...
  constructor(private _widgetService: WidgetService, private _messageService: MessageService,
    ...
    this.dailyMaintenanceTrace = false;
    this.scheduleFlag = false;
    this.screenID = 'SCR0000000046';
    this.activeIndex = 0;
  ...
  ngOnInit() {
      this.menuItems = [{
              ...
              title: 'Pick your item list file',
              command: (event: any) => {
                  this.activeIndex = 0;
                  this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Pick your data file item-list', detail: event.item.label});
              }
          },
          {
              id: 'step1',
  ...
  validationChanges() {
    ...
    if (this.checkGlobal()) {
        ...
        this.waitMessage =  'Step 1/4: Posting the execution plan... <br>'+ 
                            '<br><br>'+
                            '<b>Item description change is usually taking between 1 and 2 minutes</b>';
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                        ...
                        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                            'Step 2/4: Executing item description mapping change... <br>'+ 
                                            '<br><br>'+
                                            '<b>Item description change is usually taking between 1 and 2 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing item description mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                                ...
                                                this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                    ...
                                                    () => { 
                                                        ...
                                                        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                                            ...
                                                                            '<br><br>'+
                                                                            '<b>Item description change is usually taking between 1 and 2 minutes</b>';// Show recap dialog instead of simple dialog
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>Item codes are all recognized</li></ul>'); 
  ...
  checkGlobal(): boolean {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'ITEM_DESC') {
        this.globalError.push('The column B header must be named ITEM_DESC'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'SV_CODE') {
      this.globalError.push('The column C header must be named SV_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[3].field.toUpperCase() !== 'SV_SHORT_DESC') {
        this.globalError.push('The column D header must be named SV_SHORT_DESC'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[4].field.toUpperCase() !== 'SV_LONG_DESC') {
        this.globalError.push('The column E header must be named SV_LONG_DESC'); 
      result = false;
    }
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[6].field.toUpperCase() !== 'LV_DESC') {
        this.globalError.push('The column G header must be named LV_DESC'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[7].field.toUpperCase() !== 'LU_TYPE') {
      this.globalError.push('The column H header must be named LU_TYPE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[8].field.toUpperCase() !== 'LU_DESC') {
        this.globalError.push('The column I header must be named LU_DESC'); 
      result = false;
    }
