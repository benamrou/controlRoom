 ...
 */
@Component({
    selector: 'item-logistic-code',
    templateUrl: './item.logistic.code.component.html',
    providers: [WarehouseService, WidgetService, MessageService, ExportService, ImportService],
    styleUrls: ['./item.logistic.code.component.scss', '../../../app.component.scss'],
...
export class ItemLogisticCodeComponent implements OnInit{
  ...
  private buildMassUpdateSteps(): void {
    this.menuItems = buildMassUpdateMenuItems(this._labels, this._labels.text('S28.MU.STP0', 'Select your Item logistic code file change.'));
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>Item/LV codes are all recognized</li>' +
                                                            ' <li>Item logistic types and codes are all recognized</li></ul>'); 
  ...
  checkGlobal(): boolean {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'LV_CODE') {
        this.globalError.push('The column B header must be named LV_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'UNIT_LEVEL') {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[4].field.toUpperCase() !== 'LOGISTIC_CODE') {
        this.globalError.push('The column E header must be named LOGISTIC_CODE'); 
      result = false;
    }
