...
@Component({
    ...
    styleUrls: ['./supplier.address.component.scss', '../../../app.component.scss'],
...
export class SupplierAddressComponent implements OnInit{
  ...
  constructor(private _widgetService: WidgetService, private _messageService: MessageService,
    ...
    this.itemTrace = true; /* By default insert trace */
    this.scheduleFlag = false;
    this.screenID = 'SCR0000000038';
    this.activeIndex = 0;
  ...
  private buildMassUpdateSteps(): void {
    this.menuItems = buildMassUpdateMenuItems(this._labels, this._labels.text('S38.MU.STP0', 'Select your Supplier address file change.'));
                ...
                () => { 
                        if (existTemplate) {
                            this._messageService.add({key:'top', sticky:true, severity:'success', summary:'Template file', detail:  
                                                    'File Supplier address downloaded.' }); 
                        } else {
  ...
  validationChanges() {
    ...
    if (this.checkGlobal()) {
        this.waitMessage =  'Step 1/4: Posting the execution plan...<br>'+ 
                            '<br><br>'+
                            '<b>Supplier address change is usually taking between 1 and 2 minutes</b>';
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                        ...
                        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                            'Step 2/4: Executing supplier address mapping change...<br>'+ 
                                            '<br><br>'+
                                            '<b>supplier address change is usually taking between 1 and 2 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing supplier address mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => {  
                                                    ...
                                                    () => { 
                                                        ...
                                                        this.msgFinalDisplayed = 'Item - supplier address  ' + this.uploadedFiles[0].name + ' - ' + 
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>supplier address codes are all recognized</li></ul>'); 
  ...
  checkGlobal(): boolean {
    this.globalError=[];
    let result = true;
    if (this._importService.wb.sheets[0].worksheet.columns[0].field.toUpperCase() !== 'SUPPLIER_CODE') {
      this.globalError.push('The column A header must be named SUPPLIER_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'ADDRESS_CHAIN') {
        this.globalError.push('The column B header must be named ADDRESS_CHAIN'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'ORDER_FROM_ADDRESS') {
        this.globalError.push('The column C header must be named ORDER_FROM_ADDRESS'); 
      result = false;
    }
