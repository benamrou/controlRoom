...
@Component({
    selector: 'itemlist-desc',
    templateUrl: './itemlist.description.component.html',
    styleUrls: ['./itemlist.description.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class ItemListDescriptionComponent implements OnInit{
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                        ...
                        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                            'Step 2/4: Executing item list description mapping change... <br>'+ 
                                            '<br><br>'+
                                            '<b>Item list description change is usually taking between 1 and 2 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing item list description mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>Item list codes are all recognized</li></ul>'); 
  ...
  checkGlobal(): boolean {
    this.globalError=[];
    let result = true;
    if (this._importService.wb.sheets[0].worksheet.columns[0].field.toUpperCase() !== 'ITEM_LIST') {
      this.globalError.push('The column A header must be named ITEM_LIST'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'NEW_DESCRIPTION') {
        this.globalError.push('The column B header must be named NEW_DESCRIPTION'); 
      result = false;
    }
