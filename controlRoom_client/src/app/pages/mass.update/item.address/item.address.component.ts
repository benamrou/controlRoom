...
@Component({
    ...
    styleUrls: ['./item.address.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class ItemAddressComponent implements OnInit{
  ...
  ngOnInit() {
      this.menuItems = [{
              ...
              title: 'Pick your item address file',
              command: (event: any) => {
                  this.activeIndex = 0;
                  this._messageService.add({key:'top', sticky:false, severity:'info', summary:'Pick your data file item address', detail: event.item.label});
              }
          },
          {
              id: 'step1',
                                ...
                                () =>    {  
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => { this._messageService.add({key:'top', sticky:false, severity:'info', summary:'Step 4/4: Executing plan', detail:  '"' + this.uploadedFiles[0].name + '" processing plan results collected.'});
                                                    this.msgFinalDisplayed = 'item address  ' + this.uploadedFiles[0].name + ' - ' + 
                                                    ...
                                                    () => { 
                                                        ...
                                                        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                                            ...
                                                                            '<br><br>'+
                                                                            '<b>item address change is usually taking between 5 and 10 minutes</b>';// Show recap dialog instead of simple dialog
  ...
  checkGlobal(): boolean {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'STORE') {
        this.globalError.push('The column B header must be named STORE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'SCHEMATIC') {
