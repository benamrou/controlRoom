 ...
 */
@Component({
    selector: 'item-end-upc',
    templateUrl: './item.end.upc.component.html',
    providers: [WarehouseService, WidgetService, MessageService, ExportService, ImportService],
    styleUrls: ['./item.end.upc.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class ItemEndUPCComponent implements OnInit{
  ...
  private buildMassUpdateSteps(): void {
    this.menuItems = buildMassUpdateMenuItems(this._labels, this._labels.text('S51.MU.STP0', 'Select your Item end barcode file change.'));
                ...
                () => { 
                        if (existTemplate) {
                            this._messageService.add({key:'top', sticky:true, severity:'success', summary:'Template file', detail:  
                                                    'File item end barcode downloaded.' }); 
                        } else {
  ...
  validationChanges() {
    ...
    if (this.checkGlobal()) {
        ...
        this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                            '<br><br>'+
                            '<b>item end barcode change is usually taking between 1 and 3 minutes</b>';
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                       ...
                       this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                           'Step 2/4: Executing item end barcode mapping change... <br>'+ 
                                            '<br><br>'+
                                            '<b>item end barcode change is usually taking between 1 and 3 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing item end barcode mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => { this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Step 4/4: Executing plan', detail:  '"' + this.uploadedFiles[0].name + '" processing plan results collected.'});
                                                    this.msgFinalDisplayed = 'item end barcode  ' + this.uploadedFiles[0].name + ' - ' + 
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>item end barcode codes are all recognized</li></ul>'); 
  ...
  checkGlobal(): boolean {
    this.globalError=[];
    let result = true;    
    if(this._importService.wb.sheets[0].worksheet.columns.length < 1) {
        this.globalError.push('item end barcode file must contains the following headers: UPC'); 
        return false;
    }
