...
@Component({
    ...
    templateUrl: './stock.layer.component.html',
    styleUrls: ['./stock.layer.component.scss', '../../../app.component.scss'],
...
export class StockLayerComponent implements OnInit{
  ...
  ngOnInit() {
      this.menuItems = [{
              ...
              title: 'Pick your stock layer file',
              command: (event: any) => {
                  this.activeIndex = 0;
                  this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Pick your data file item stock layer', detail: event.item.label});
              }
          },
          {
              id: 'step1',
  ...
  validationChanges() {
    ...
    if (this.checkGlobal()) {
        ...
        this.waitMessage =  'Step 1/4: Posting the execution plan... <br>'+ 
                            '<br><br>'+
                            '<b>Stock layer change is usually taking between 1 and 3 minutes</b>';
                    ...
                    if (this.scheduleFlag) {
                        ...
                        /** Run the job integration */
                            this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                'Step 2/4: Executing stock layer mapping change... <br>'+ 
                                                '<br><br>'+
                                                '<b>Stock layer change is usually taking between 1 and 3 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing stock layer mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => { this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Step 4/4: Executing plan', detail:  '"' + this.uploadedFiles[0].name + '" processing plan results collected.'});
                                                    this.msgFinalDisplayed = 'Stock layer  ' + this.uploadedFiles[0].name + ' - ' + 
                                                    ...
                                                    () => { 
                                                        ...
                                                        this.msgFinalDisplayed = 'Item - Stock layer  ' + this.uploadedFiles[0].name + ' - ' + 
                                                                                ' has been successfully processed.';
  ...
  checkGlobal(): boolean {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[1].field.toUpperCase() !== 'ITEM_CODE') {
        this.globalError.push('The column B header must be named ITEM_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[2].field.toUpperCase() !== 'LV_CODE') {
