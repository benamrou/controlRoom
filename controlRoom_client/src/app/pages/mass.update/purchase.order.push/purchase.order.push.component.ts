...
import { WidgetService, ExportService, ImportService } from '../../../shared/services';
 ...
 */
@Component({
    selector: 'purchase-order-push',
    templateUrl: './purchase.order.push.component.html',
    providers: [WidgetService, MessageService, ExportService, ImportService],
    styleUrls: ['./purchase.order.push.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None
})

export class PurchaseOrderPushComponent implements OnInit{
  ...
  private buildMassUpdateSteps(): void {
    this.menuItems = buildMassUpdateMenuItems(this._labels, this._labels.text('S49.MU.STP0', 'Select your Push/Breakdown order file.'));
                        ...
                        () => { 
                                ...
                                if (rowsWithError.length === 0) {
                                    ...
                                    this.globalValid.push('<i class="fas fa-thumbs-up" style="padding-right: 1em;"></i> Data file verification SUCCESSFUL ' +
                                                            ...
                                                            ' <li>Site codes are all recognized</li>' +
                                                            ' <li>Items codes are all recognized</li>' +
                                                            ' <li>Items orderability are all confirmed</li></ul>'); 
  ...
  checkGlobal(): boolean {
    ...
    if (this._importService.wb.sheets[0].worksheet.columns[3].field.toUpperCase() !== 'ORDER_MODE') {
        this.globalError.push('The column D header must be named ORDER_MODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[4].field.toUpperCase() !== 'URGENCY') {
        this.globalError.push('The column E header must be named URGENCY'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[5].field.toUpperCase() !== 'ORDER_STATUS') {
        this.globalError.push('The column F header must be named ORDER_STATUS'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[6].field.toUpperCase() !== 'SUPPLIER_CODE') {
        this.globalError.push('The column G header must be named SUPPLIER_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[7].field.toUpperCase() !== 'ITEM_CODE') {
        this.globalError.push('The column H header must be named ITEM_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[8].field.toUpperCase() !== 'LV_CODE') {
        this.globalError.push('The column I header must be named LV_CODE'); 
      result = false;
    }
    if (this._importService.wb.sheets[0].worksheet.columns[9].field.toUpperCase() !== 'QTY') {
        this.globalError.push('The column J header must be named QTY'); 
      result = false;
    }
