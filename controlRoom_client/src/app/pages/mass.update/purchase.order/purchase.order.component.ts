...
@Component({
    ...
    styleUrls: ['./purchase.order.component.scss', '../../../app.component.scss'],
...
export class PurchaseOrderComponent implements OnInit{
  ...
  ngOnInit() {
      this.menuItems = [{
              ...
              title: 'Pick your purchase order file',
              command: (event: any) => {
                  this.activeIndex = 0;
                  this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Pick your data file purchase order', detail: event.item.label});
              }
          },
          {
              id: 'step1',
    ...
    if (this.checkGlobal()) {
        ...
        this._importService.postExecution(this.uploadedFiles[0].name, this.toolID,
                    ...
                    if (this.scheduleFlag) {
                       ...
                       this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                           'Step 2/4: Executing purchase order mapping change... <br>'+ 
                                            '<br><br>'+
                                            '<b>purchase order change is usually taking between 1 and 3 minutes</b>';
                        ...
                        this._importService.execute(executionId.RESULT[0]).subscribe 
                                ...
                                () =>    {  
                                    ...
                                    this.waitMessage =  'Step 1/4: Posting the execution plan... &emsp;<b>COMPLETED</b><br>'+ 
                                                        'Step 2/4: Executing purchase order mapping change... &emsp;<b>COMPLETED</b><br>'+ 
                                    ...
                                    this._importService.executePlan(userID, this.toolID).subscribe( 
                                            ...
                                            () => { this._messageService.add({key:'top', sticky:true, severity:'info', summary:'Step 4/4: Executing plan', detail:  '"' + this.uploadedFiles[0].name + '" processing plan results collected.'});
                                                    this.msgFinalDisplayed = 'purchase order  ' + this.uploadedFiles[0].name + ' - ' + 
  ...
  checkGlobal(): boolean {
    ...
    if(this._importService.wb.sheets[0].worksheet.columns.length < 8) {
        this.globalError.push('purchase order file must contains the following headers: SITE_CODE, ORDER_DATE, DELIVERY_DATE, ORDER_STATUS, ITEM_CODE, LV_CODE, QTY'); 
        return false;
    }
