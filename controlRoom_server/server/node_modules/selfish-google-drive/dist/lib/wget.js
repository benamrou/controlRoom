...
const downloadBinaryFile = (token, file) => {
  ...
  return new Bluebird((resolve, reject) => {
    request.get(`${constant.wget}/${fileId}`).query({
      alt: 'media'
    }).set('Accept', 'application/json').set('Authorization', `Bearer ${accessToken}`).on('response', response => {
...
const exportGD = (token, file) => {
  ...
  return new Bluebird((resolve, reject) => {
    request.get(`${constant.lsUrl}/${fileId}/export`).query({
      mimeType: exportTo
    }).set('Accept', 'application/json').set('Authorization', `Bearer ${accessToken}`).on('response', response => {
      if (response.status >= 400) {
        reject(response);
