...
function json2xls(workbook, worksheet, alertData, detailData, extensionHeader, tableName, formatingRules, renameColumn) {
    ...
    if (alertData[0].ALTCOLMOVE) {
        ...
        // 1. Capture the elements in the exact order requested, using original indices
        let movedElements = colMove.move2end.map(idx => valueColumns[idx]);

        // 2. Remove those indices safely (descending order avoids re-indexing issues)
        let indicesToRemove = [...colMove.move2end].sort((a, b) => b - a);
        for (let idx of indicesToRemove) {
            valueColumns.splice(idx, 1);
        }

        // 3. Append captured elements to the end, preserving the order given in move2end
        valueColumns.push(...movedElements);
    }
