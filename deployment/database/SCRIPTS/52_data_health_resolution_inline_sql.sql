-- =============================================================================
-- 52_data_health_resolution_inline_sql.sql
-- Data Health — inline resolution UPDATE/MERGE on AI_DATA_CHECK_DEF + SQL_JOB mode
-- Deploy after: 49–51 (resolution columns + AI0000091). Re-runnable.
--
-- Admins store row-level fix SQL in RESOLUTION_SQL (CLOB, max ~4000 via JSON POST).
-- Placeholders :BIND_NAME must match RESOLUTION_PARAM_MAP (bind=Column header).
-- Modes: SQL (per-row SQL only), SQL_JOB (per-row SQL then DBMS_SCHEDULER job).
-- Combined SQL + GOLD script: deploy 96_data_health_resolution_sql_script_modes.sql (SQL_SCRIPT, SQL_SCRIPT_JOB).
-- Execution: AI0000092 (not a separate LIBQUERY per check).
-- =============================================================================

BEGIN
    EXECUTE IMMEDIATE 'ALTER TABLE AI_DATA_CHECK_DEF ADD (RESOLUTION_SQL CLOB)';
EXCEPTION
    WHEN OTHERS THEN
...
COMMENT ON COLUMN AI_DATA_CHECK_DEF.RESOLUTION_SQL IS
  'Row-level UPDATE/MERGE for SQL / SQL_JOB modes. Use :bind tokens; map via RESOLUTION_PARAM_MAP (bind=Column).';
...
-- ── Execute inline SQL from check definition + binds in POST body ─────────────
...
BEGIN
    ...
    LOOP
        ...
        v_val  := json_val(v_col);
        IF v_val IS NULL OR TRIM(v_val) IS NULL THEN
            DBMS_SQL.CLOSE_CURSOR(v_cur);
            RAISE_APPLICATION_ERROR(-20099,
              'Missing bind value for column "' || v_col || '" (bind :' || v_bind || ').');
        END IF;
...
-- ── AI0000092 — POST row fix using RESOLUTION_SQL on AI_DATA_CHECK_DEF ───────
DELETE FROM LIBQUERY WHERE QUERYNUM = 'AI0000092';
...
SELECT (SELECT NVL(MAX(QUERYID), 0) + 1 FROM LIBQUERY),
       'AI0000092',
       'S24 — execute inline resolution SQL',
       'Runs RESOLUTION_SQL for SQL/SQL_JOB. Body: {values:[{CHECK_ID,RETAILER_ID,binds…,DRY_RUN?:1}]}.',
       q'~
DECLARE
  v_dry NUMBER(1);
BEGIN
  SELECT NVL(jt.DRY_RUN, 0)
    INTO v_dry
    FROM REQUEST_QUERY_BODY rb,
         JSON_TABLE(rb.REQUESTBODY, '$.values[0]'
           COLUMNS (
             DRY_RUN NUMBER PATH '$."DRY_RUN"'
           )
         ) jt
   WHERE rb.REQUESTID = :param1;

  AI_EXECUTE_CHECK_RESOLUTION_SQL(:param1, v_dry);
END;
~',
...
-- ── Refresh reads / upsert to include RESOLUTION_SQL ──────────────────────────
