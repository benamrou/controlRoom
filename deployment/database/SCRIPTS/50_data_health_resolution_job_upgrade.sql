-- =============================================================================
-- 50_data_health_resolution_job_upgrade.sql
-- Data Health — resolution mode: LIBQUERY | JOB | LIBQUERY_JOB (sequential)
-- Deploy after: 49_data_health_resolution_upgrade.sql
...
BEGIN
    EXECUTE IMMEDIATE q'[
        ...
        CHECK (RESOLUTION_MODE IN ('NONE','LIBQUERY','JOB','LIBQUERY_JOB'))
    ]';
EXCEPTION
    WHEN OTHERS THEN
...
COMMENT ON COLUMN AI_DATA_CHECK_DEF.RESOLUTION_MODE IS
  'Fix pipeline: NONE | LIBQUERY (row DML) | JOB (DBMS_SCHEDULER) | LIBQUERY_JOB (LIBQUERY then job).';
COMMENT ON COLUMN AI_DATA_CHECK_DEF.RESOLUTION_JOB_NAME IS
  'DBMS_SCHEDULER job name (optionally schema-qualified) for JOB / LIBQUERY_JOB step 2.';
...
-- ── AI0000091 — Run configured scheduler job for a check (step 2 or JOB-only) ─
...
SELECT (SELECT NVL(MAX(QUERYID), 0) + 1 FROM LIBQUERY),
       ...
       'Runs RESOLUTION_JOB_NAME when RESOLUTION_MODE is JOB or LIBQUERY_JOB. Body: {values:[{CHECK_ID,RETAILER_ID}]}.',
       q'~
DECLARE
...
BEGIN
  ...
  IF v_mode NOT IN ('JOB', 'LIBQUERY_JOB') THEN
...
~',
       ...
       'CHECK_ID,CHECK_CODE,CHECK_NAME,CHECK_DESCRIPTION,QUERY_NUM,TIER,SEVERITY,ENABLED,SKILL_CODE,ENTITY_KEY,DISPLAY_ORDER,RESOLUTION_MODE,RESOLUTION_QUERY_NUM,RESOLUTION_PARAM_MAP,RESOLUTION_JOB_NAME,FIXABLE_STATUS_COLUMN,FIXABLE_STATUS_VALUE,ISSUE_COUNT,LAST_RUN_AT,STATUS,TREND',
       1, 0, 0
  FROM dual;
...
~',
       ':param1=retailer_id',
       'CHECK_ID,CHECK_CODE,CHECK_NAME,CHECK_DESCRIPTION,QUERY_NUM,TIER,SEVERITY,ENABLED,RETAILER_ID,SKILL_CODE,ENTITY_KEY,DISPLAY_ORDER,RESOLUTION_MODE,RESOLUTION_QUERY_NUM,RESOLUTION_PARAM_MAP,RESOLUTION_JOB_NAME,FIXABLE_STATUS_COLUMN,FIXABLE_STATUS_VALUE,CREATED_AT,UPDATED_AT',
       1, 0, 0
  FROM dual;
...
~',
       ':param1=check_id',
       'CHECK_ID,CHECK_CODE,CHECK_NAME,CHECK_DESCRIPTION,QUERY_NUM,TIER,SEVERITY,ENABLED,RETAILER_ID,SKILL_CODE,ENTITY_KEY,DISPLAY_ORDER,RESOLUTION_MODE,RESOLUTION_QUERY_NUM,RESOLUTION_PARAM_MAP,RESOLUTION_JOB_NAME,FIXABLE_STATUS_COLUMN,FIXABLE_STATUS_VALUE',
       1, 0, 0
  FROM dual;
