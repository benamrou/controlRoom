...
SELECT (SELECT NVL(MAX(QUERYID),0)+1 FROM LIBQUERY),
       ...
       ':param1=skill_id',
       'STEP_ORDER,TEMPLATE_CODE,STEP_LABEL,STOP_FIELD,STOP_OPERATOR,STOP_VALUE,CONCLUSION_KEY',
       1, 0, 0
  FROM dual;
...
SELECT (SELECT NVL(MAX(QUERYID),0)+1 FROM LIBQUERY),
       ...
       'Insert or update a diagnostic chain step row. POST body keys: SKILL_ID,STEP_ORDER,TEMPLATE_CODE,STOP_FIELD,STOP_OPERATOR,STOP_VALUE,CONCLUSION_KEY,STEP_LABEL.',
       q'~MERGE INTO AI_DIAGNOSTIC_STEP t
         ...
         WHEN MATCHED THEN UPDATE SET
             ...
             t.STEP_LABEL = s.STEP_LABEL
         WHEN NOT MATCHED THEN INSERT
             (SKILL_ID,STEP_ORDER,TEMPLATE_CODE,STOP_FIELD,STOP_OPERATOR,STOP_VALUE,CONCLUSION_KEY,STEP_LABEL)
         VALUES
             (s.SKILL_ID,s.STEP_ORDER,s.TEMPLATE_CODE,s.STOP_FIELD,s.STOP_OPERATOR,s.STOP_VALUE,s.CONCLUSION_KEY,s.STEP_LABEL)~',
       ':param1=REQUESTID',
       'MERGED',
       0, 0, 1
  FROM dual;
